...
window.onload = async function () {
    ...
    const showInternetAlert = () => {
        showPopupMessage("It seems the page is taking too long to load. Please check your internet connection.");
    ...
    try {
        ...
        if (clientData) {
           ...
           // document.getElementById("profilePic").src = imgSrc;
            ...
            document.getElementById("profileview").src = imgSrc;
            document.getElementById("profilemain").src = imgSrc;
            }
...
function logindone(){
    ...
    draft.style.display = "block";
    viewBoxs.style.display = "block";
    frontNamediv.style.display = "block";
...
async function oldmodifyUserDetails(event) {
  event.preventDefault(); // Prevent form submission
  ...
  if (!newUsername && !newPassword) {
    ...
    return false;
            ...
            reader.onload = async function (event) {
                const response = await fetch(API_URL, {
                    method: "POST",
                    body: JSON.stringify({ action: "update", email, fileName: file.name, fileData: event.target.result })
                });
                const result = await response.json();
                showPopupMessage(result.message, result.status);
                if (result.status === "success");
             //   profilePic.src = previewImage.src;
                previewview.src = previewImage.src;
                previewmain.src = previewImage.src;
...
async function openEditUserModal() {
  ...
  try {
    ...
    const response = await fetch(API_URL);  // Use the correct API URL
    const data = await response.json();
...
const popup = document.getElementById('id04');
const profileInput = document.getElementById('profileInput');
const previewImage = document.getElementById('previewImage');
const previewview = document.getElementById('profileview');
const previewmain = document.getElementById('profilemain');
...
//const GOOGLE_TEST_URL = "https://script.google.com/macros/s/AKfycbxze3_dDVa_hV70v23hZJjTCZqidigubb5DKxhvGEp-4IC96j1L-cWh7lFmsJ2o4UDk4g/exec"; // Replace with your Apps Script URL
...
async function updateFile() {
            ...
            if (!file) return alert("Please select a file to update.", "error");
...
async function modifyUserDetails(event) {
  ...
  // If no changes were made, show "Up to date" message
  if (!newUsername && !newPassword && !file) {
    showPopupMessage("Already up to date.");
    return;
  }
  ...
  async function updateUserDetails() {
    if (!newUsername && !newPassword) return; // Skip if nothing is changing
  ...
  if (file) {
    ...
    reader.onload = async function (event) {
      try {
        ...
        // After successful image upload, update username/password if changed
        if (newUsername || newPassword) {
          await updateUserDetails();
        }
      } catch (error) {
        console.error("Error uploading profile picture:", error);
        showPopupMessage("Image upload failed. Please try again.");
      } finally {
        hideLoading();
      }
    };
    ...
    // If no file selected, update username and password only
    await updateUserDetails();
    hideLoading();
  }
}
